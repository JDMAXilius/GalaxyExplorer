...
|---|---|---|---|
| `galaxy_view_scene` | `GalaxyContent` | `milky_way_prefab` (DrawStars ×3 layers), `galaxy_pois_prefab` (the original POI markers: seven `poi_prefab` nebula markers with `CardPOI` — hover card, click travels — and the `solar_system` large marker with `PlanetPOI`; `galactic_center` sits loose in the scene; the `destination_tags` node of 9 `LabelButton` tags was removed by CS-170) | Milky Way |
