...
| Stereo | As configured today: Android **Single Pass Instanced**, Windows/Link **Multi Pass** (§7.1) | Not all custom shaders carry the macros — 36 of 47 did not, which breaks one eye on-device; see `docs/SHADER_STEREO_AUDIT.md` (CS-095) |
...
Assets/
  scripts/                 99 root scripts (original app) + folders below
    ...
    interaction/ [planned] FreePlacementSolver, LabelButton, DestinationTags, TwoHandTransformer
...
- `ManipulationHandler`: one/two-hand move/rotate/scale on a `hostTransform`; `OnManipulationStarted/Ended`; used by `ForceSolver` in the Manipulation state. `PostManipulationResetter` optionally animates rotation/scale/position back after release. `HostTransform` and `ManipulationType` are settable at run time (CS-108) so a *handle* can retarget a handler at the thing it drags and shut two-handed scaling off; setting the host mid-grab re-captures the offsets, which were measured against the old host.
...
- **The dock's drag bar** (CS-108) is the fourth user of that router and the only one that is a *handle*: `drag_bar` carries `BoxCollider` (60 × 8 × 2 mm, widened from the 1.6 mm it draws so a pinch can find it) + `GEInteractable` + `ManipulationHandler` + `ManipulationPointerRouter`, with `hostTransform` pointed at the **dock root** and `OneHandedOnly`, both written by `UiPrefabBuilder.BuildDock` and re-asserted by `DockController` on `Start` because the committed prefab predates them. `DockController.LateUpdate` then holds the two parts of the pose the dock owns while the handler drives the position: `FaceThePlayer()` (GDD §8.1's re-tilt, on the release frame as well) and the `localScale` captured at grab, so a wrist cannot roll the dock and no gesture can resize its millimetre canvas. `LateUpdate`, not a flag on the handler, because it runs after every `Update` and cannot lose that race. Palm-up show/hide is suppressed during a drag — hiding the dock switches the bar off and would end the grab. On the desktop the left drag arrives through the same routing; `DesktopMouseInput.ManipulatedHandler` now skips `OneHandedOnly` handlers, so right-drag and the wheel stay on the view rather than rolling or resizing the dock.
- `experience/FreePlacementAnchor` (CS-107): the other half of the same split — the only thing that brings a moved `ManipulationHandler` object **no `ForceSolver` and no `LayoutRig` owns** back home. It remembers a **local** TRS (the frame moves: prefab content hangs off the `ViewLoader`) captured by the spawner, and eases back over 0.8 s. `ExperienceDirector` calls the static `FreePlacementAnchor.CaptureHome(Transform)` immediately after `Instantiate` and **before `GrowIn`**, which drives `localScale` from zero every frame and would otherwise be what got remembered; that call also *adds* an anchor where one is missing, using the `ManipulationPointerRouter` guard (`HostTransform` with no `ForceSolver` on it or above it), so content from a builder nobody has re-run still comes home. `RestoreAll()` is called from `DesktopMouseInput.RestoreLayout` (`R`, above both existing passes, because the rig pass returns) and `ResetView` (every desktop route to Recenter); `RestoreAllImmediate()` from `ExperienceDirector.RestoreEverything`. It has no `Update`: GDD §5.2's stray auto-restore is a body rule and these are objects meant to be pushed away. The world dock's `DockController.Recenter` calls `RestoreAll()` too, **before** its own camera guard, so a missing camera cannot strand an overlay the player pushed across the room.
- `experience/FreePlacementAnchor` (CS-107): the other half of the same split — the only thing that brings a moved `ManipulationHandler` object **no `ForceSolver` and no `LayoutRig` owns** back home. It remembers a **local** TRS (the frame moves: prefab content hangs off the `ViewLoader`) captured by the spawner, and eases back over 0.8 s. `ExperienceDirector` calls the static `FreePlacementAnchor.CaptureHome(Transform)` immediately after `Instantiate` and **before `GrowIn`**, which drives `localScale` from zero every frame and would otherwise be what got remembered; that call also *adds* an anchor where one is missing, using the `ManipulationPointerRouter` guard (`HostTransform` with no `ForceSolver` on it or above it), so content from a builder nobody has re-run still comes home. `RestoreAll()` is called from `DesktopMouseInput.RestoreLayout` (`R`, above both existing passes, because the rig pass returns) and `ResetView` (every desktop route to Recenter); `RestoreAllImmediate()` from `ExperienceDirector.RestoreEverything`. It has no `Update`: GDD §5.2's stray auto-restore is a body rule and these are objects meant to be pushed away. `DockController.Recenter` calls it too, **above its own camera guard**, so a missing camera stops the dock re-parking without also stranding a galaxy the player pushed across the room.
...
- **[planned]** `InfoPanel` (rename/extend `PlanetInfoCard`: body/scene/moon variants, superscript mass, outline material), `DockController` (world-space canvas from Figma export; tiles bound to modules; `XRPokeFilter` buttons; grab bar = a `ManipulationHandler` **on the bar** whose host is the dock root, which is how CS-108 built it — the handler cannot live on the root itself, because then any bare collider anywhere in the dock would drag it; palm-up show/hide via `XRInputRig` palm normal), `DockPopup`, `MoonLabel`, `DesktopDock`.
- **[planned]** `InfoPanel` (rename/extend `PlanetInfoCard`: body/scene/moon variants, superscript mass, outline material), `DockController` (world-space canvas from Figma export; tiles bound to modules; `XRPokeFilter` buttons; grab bar = `ManipulationHandler` on the dock root; palm-up show/hide via `XRInputRig` palm normal), `DockPopup`, `MoonLabel`, `DesktopDock`.
- **[built, CS-108]** The dock's **drag bar** (GDD §8.1, contract row F-06). `UiPrefabBuilder.BuildDock` gives `drag_bar` a `BoxCollider` + `GEInteractable` + `ManipulationHandler` + `ManipulationPointerRouter`, and — the part that makes it a dock handle rather than a loose bar — serializes the handler's `hostTransform` to the **dock root**. Mode is `OneHandedOnly`: GDD §8.1 gives the dock no scale and no free rotation, and `Recenter` writes position and rotation only, so a dock a two-handed pinch had scaled would have no way back to its authored size. `playGrabSounds` is on, because no `ForceSolver` sounds this grab. The re-tilt is `DockController.LateUpdate` → `FaceThePlayer()`, which overwrites the wrist rotation the handler wrote in `Update` — both it and `Recenter` now aim the dock through the same two-step `Flatten` + `ParkedRotation` pair, so a dragged dock and a parked one cannot end up at different angles. `Recenter` drops an in-progress grab (disable/enable the handler, which clears its pointers) before parking, or the next frame of the drag would pull the dock straight back out. `DockController` refuses a bar whose `HostTransform` is not the dock root — it logs and disables the handler — so a `dock_prefab` nobody has rebuilt keeps the old dead bar instead of tearing the bar off the plate. **Desktop: none.** The world dock parks 0.72 m below the eye line 0.75 m out, which is outside any sane frustum (it would need a ~88° vertical FOV), and the mouse `GEPointer` is a camera ray — the bar is unreachable there by construction, and `DesktopDock` is a screen-space mirror with no world pose to drag. `Recenter` is the desktop's only dock placement control, and it already has one.
...
- **[built, CS-050; switched off by CS-170 — the original markers are the map again, and the `destination_tags` node is inactive in the prefab]** `DestinationTags` (`Assets/scripts/experience/DestinationTags.cs`) — the receiving end of the Milky Way map's nine destination tags (GDD §4.3). `LabelButton` had always drawn and clicked correctly and then raised `OnPicked`, and **nothing anywhere subscribed to that event**, which is the whole reason the map's destinations never opened. One `DestinationTags` owns the tags in a view and dispatches **on the module**: a module with a `SceneName` (`solar_system`, `sagittarius_a`) goes through `ExperienceDirector.Switch`, the same call `DockController.Choose` makes, so a tag and a tile cannot drift apart; a module with only a `ContentPrefab` (the seven nebulae) opens as an overlay over the map through `ExperienceDirector.OpenDestination`; a module with neither answers through `SwitchNotice.NotReady`. Picking the open destination again closes it, and `Esc` closes it on the desktop (added to `DesktopMouseInput.CloseTopmost`, **above** the panel sweep, which would otherwise hide the destination's own panel and leave the nebula with nothing naming it). Selection is event-driven, not polled: the director now also exposes `OpenDestinationModule`, `HasOpenDestination` and a static `DestinationChanged` — separate from `ExperienceChanged` because a destination does *not* change the place, so a tag watching the older event would light up and never learn it had closed. **Where the overlay opens is the head, not the tag**: GDD §4.3 puts the halo 1.0 m in front of the player, and it has to, because a 70 cm cloud spawned at a tag would be half buried in a 1.6 m disc. The diameter passed to `OpenDestination` is **measured off the content prefab's own grab sphere** (0.7 m for a nebula, the card width `NebulaPrefabBuilder` sized it to) rather than authored twice; `BlackHalo` multiplies it by 1.6 itself, so it must be the subject's width and not the halo's.
- **[built, CS-050]** `DestinationTagBuilder` (**Cosmic Simulation → Build Destination Tags**) writes the nine tags into a `destination_tags` node inside `galaxy_pois_prefab` — not a new prefab, because that prefab is already referenced from `galaxy_view_scene` (so no scene edit is needed), because the old markers' placements are already in that frame, and because the tags then fade and unload with the map. Idempotent: the node is destroyed and rebuilt whole. **The placement frame is a trap worth reading twice.** `POIs` has local scale (0.4, 2.5, 0.4) and `GrabArea` above it in the scene has (2.5, 0.4, 2.5), with no rotation or offset between them, so the product is exactly the identity and a child of `POIs` is in undistorted metres of the galaxy's frame. The tag node is therefore left at scale 1 and looks squashed in prefab mode — rescaling it to “fix” that would shear the tags in the only place they are ever seen. Seven of the nine anchors are lifted verbatim off existing markers (six from the prefab, `sagittarius_a` from the loose `galactic_center` marker in the scene, which is at the origin — the one honest position); Helix and Orion are new and fill the two largest gaps in bearing at comparable radii. **None of them are astrometric** and the original six never were. Each tag is a `label_button_prefab` instance lifted 0.12 m (0.17 on the two short radii, 0.22 for the galactic centre) straight above its point, with `Billboard` on `PivotAxis.Y` so it stays upright and readable, and the hairline leader sized once in canvas units — vertical in canvas space is vertical in the room when the only rotation is about Y, so `LabelButton.PointLeaderAt` is not used. The collider is deepened from 2 mm to 8 mm, the same widening and the same reason as CS-108's drag bar.
...
|---|---|---|---|
| `galaxy_view_scene` | `GalaxyContent` | `milky_way_prefab` (DrawStars ×3 layers), `galaxy_pois_prefab` (the original POI markers: seven `poi_prefab` nebula markers with `CardPOI` — hover card, click travels — and the `solar_system` large marker with `PlanetPOI`; `galactic_center` sits loose in the scene; the `destination_tags` node of 9 `LabelButton` tags is kept but inactive, CS-170) | Milky Way |
...
- **Stereo render mode, as actually configured** (`Assets/XR/Settings/OpenXR Package Settings.asset`): Android = **Single Pass Instanced** (`m_renderMode: 1`), Standalone/Link = **Multi Pass** (`m_renderMode: 0`). Confirmed 12 Sep 2026 against the enum itself, which is `MultiPass = 0, SinglePassInstanced = 1` (`OpenXRRenderSettings.cs:16-27`, vendored in `Library/PackageCache/com.unity.xr.openxr@b5b77b4027be`). An earlier revision of this section had it inverted and drew the opposite conclusion; the corrected reading is that **the missing-stereo-macro bug breaks one eye on the shipping Quest build and is invisible on Link**, where each eye gets its own draw. That is why it survived: the one configuration that would show it is the one nobody inspects per-eye, and desktop mono cannot show it at all. **Per-eye verification must run on the standalone Android build, or on Link explicitly forced to single-pass instanced.** **Full audit of all 78 shaders, with what was fixed and what still needs a compiler: `docs/SHADER_STEREO_AUDIT.md` (CS-095, CS-096).**
...
- Who plays what: `GEButton` (select, falling back to `AudioId.Select` when a prefab assigned no `clickSound`; poke release), `DockTile`, `LabelButton`, `DockPopup`, `ForceSolver` (the whole force/grab/release set, from its state machine). `ManipulationHandler` has an opt-in `playGrabSounds`, **off by default**: `ExecuteHierarchy` delivers to every enabled handler on the first matching GameObject, and on a body the handler shares that object with the `ForceSolver` that already sounds those states. Turn it on only where there is no solver — i.e. wherever `ManipulationPointerRouter` is: nebula overlays, cosmic web, Andromeda, and the dock drag bar (CS-108 — `UiPrefabBuilder` writes `hostTransform` to the dock root, `manipulationType: 0` and `playGrabSounds: 1` on `drag_bar`; confirmed live in the rebuilt `dock_prefab`, not stale; `DockController` deliberately does not also play it, which would double it).
- Who plays what: `GEButton` (select, falling back to `AudioId.Select` when a prefab assigned no `clickSound`; poke release), `DockTile`, `LabelButton`, `DockPopup`, `ForceSolver` (the whole force/grab/release set, from its state machine). `ManipulationHandler` has an opt-in `playGrabSounds`, **off by default**: `ExecuteHierarchy` delivers to every enabled handler on the first matching GameObject, and on a body the handler shares that object with the `ForceSolver` that already sounds those states. Turn it on only where there is no solver — i.e. wherever `ManipulationPointerRouter` is: nebula overlays, cosmic web, Andromeda, and the dock drag bar (CS-108).
...
- Galaxy POI cards (`CardPOI`, `poi_text_card_*` textures) were retired in code by CS-050 and are **the product's Milky Way map again as of CS-170 (14 Sep 2026)**: `CardPOI` opens its card on focus enter, closes it on focus exit after a short grace, and travels through `ExperienceDirector.Switch` on pointer down; `PlanetPOI` travels the same way when a module claims its scene. The `keepLegacyCards` switch is gone. **Nothing here can be deleted**, and a GUID sweep of `Assets/` + `ProjectSettings/` is why: `CardPOIManager` is referenced by `core_systems_scene` *and* used by `PointOfInterest` (the base class `PlanetPOI` also derives from) and by `DesktopMouseInput.CloseTopmost`; `POIBehavior` is referenced by `poi_prefab` and looked up by `PlanetPOI`; `poi_prefab` is referenced by `poi_prefab_large` (a variant of it), `galaxy_pois_prefab` and `projector_poi_prototype.unity`; `poi_prefab_large` is referenced by `galaxy_pois_prefab` and by `galaxy_view_scene` itself, where the `galactic_center` marker sits loose in the scene rather than in the POI prefab. Removing the components and the two base prefabs is scene and prefab surgery — the same shape of job as CS-088.
