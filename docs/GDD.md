...
- **Destination tag:** dark rounded card (radius 4 mm), **60 × 24 mm minimum**, carrying two lines — the name in white 5 mm, and under it a second line in caps, 3.6 mm, in the secondary ink; a thin leader line runs to the point. The card's **width is fitted to whichever line is longer** and never goes below 60 mm. Hover: +15 % scale, cyan fill, both lines darken. Selected: cyan outline.
  ...
  - **On the Milky Way map the card is drawn ×4** (v1.3, 12 Sep 2026). Those millimetres are the card's own; they are not what the eye gets. A destination tag is a child of the map, and the map is a 1.6 m object the player stands back from, so a 60 × 24 mm card is under 4 % of the disc and its 5 mm name subtends about a fifth of a degree — three or four pixels. Text in a headset wants about a degree, which at 1.2 m is 21 mm. For scale, the two inherited markers the player is asking for set their name at an em of roughly 85 mm in that same frame; twelve cards cannot all be that, so ×4 is the compromise and the one number to turn. The multiplier lives on the tag's root transform, so the card's internal layout stays in the millimetres above.
  - **Cards above the disc are stacked in four heights**, ordered outermost-lowest in steps of a card height + 2 mm (on the map, so ×4 as well), wrapping after the fourth. The cards billboard, so two at the same height overlap whenever the player is near the line joining them, however far apart they are on the map — one height each would be the only complete answer, but at the size the cards now have that is a 1.1 m tower over a 1.6 m disc. Wrapping keeps the spread at 0.29 m and puts most of the disc between any two cards that do share a height. The complete answer is the inherited markers' own: set each card off to one side on a long angled leader instead of straight up.
  - **A card's foot stays on the disc.** The three outside galaxies are placed on their real galactic bearings from the Sun at a made-up range; a range that would carry the foot past the rim is shortened so the leader lands on the map rather than in empty space beside it.
