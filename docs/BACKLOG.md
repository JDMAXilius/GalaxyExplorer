...
| CS-196 | CC | Settings window, owner's direction 16 Sep: add a **Microphone** row (‹ device ›, cycling `Microphone.devices` into `BeingMic.Device`, with a live level bar) and a two-tap **Quit** to `UtilityWindow`; set Mute and Narration side by side; the window grows to 120 × 150 mm; the desktop dock's Mute button becomes **Settings** (`M` still mutes). Mockup: https://claude.ai/artifact/QwVdM4XzMXSSSr7qXJjugr. Update `docs/ui/spec.md` §5 and GDD §8.2 | CS-194 | todo (waiting on the owner's sign-off on the mockup) |
