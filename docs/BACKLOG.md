...
| CS-033 | TERM | `smoke.cs` harness: switch all modules, panels, place/restore, screenshots, console clean | CS-032 | doing |
...
| CS-037 | CC | `ExperienceDirector`: clear `_switching` in a `finally` plus a timeout, so a scene that fails to load cannot wedge the dock in an emptied room | CS-035 | todo |
| CS-038 | CC | Refuse or queue `Switch` while the intro flow is running; wire `OpenStartModule`, which today has no callers at all | CS-032 | todo |
