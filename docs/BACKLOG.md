...
| CS-170 | CC | The Milky Way map goes back to the original Galaxy Explorer markers (angled leader line, name label, picture card) for all nine destinations, with Helix and Orion added as markers; **hover opens the card, click travels** through `ExperienceDirector.Switch`; the `LabelButton` tag set is switched off, not deleted | CS-168 | done (by reading; needs the editor pass in its note) |
...
*What changed.* The markers were never gone: `galaxy_pois_prefab` still held the five `poi_prefab` instances and the `solar_system` large marker, `galaxy_view_scene` the loose `galactic_center` one; CS-050 had switched them off in `CardPOI.Awake` and `LegacyPoiCleanup` had deactivated the two large ones. The switch is gone from `CardPOI`, the two large markers are active again (the prefab override and the scene override), and **two new `poi_prefab` instances, `poi_helix` and `poi_orion`,** are written into the prefab at the positions `DestinationTagBuilder` gave those two tags, with the nebulae plates (`Assets/Textures/nebulae/helix_texture.jpg`, `orion_texture.jpg`) as their card photographs, their module narration as `CardAudio`, and distance / size / age texts. Their pointing-line positions are not baked into the prefab because `DynamicLine` rewrites them every frame. The `destination_tags` node is set inactive in the prefab and left in place so **Build Destination Tags** and `DestinationTags` still work if the owner wants the tags back.
