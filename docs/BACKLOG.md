...
| CS-176 | TERM | Build the being: **Cosmic Simulation → Build Cosmic Being** (writes `Assets/prefabs/being/cosmic_being_prefab.prefab`, the two materials, `Assets/data/being/cosmic_being_settings.asset`, and wires `beingPrefab` into both dock prefabs), then **Build UI Prefabs** and **Build Desktop Dock** for the new button; compile clean; tune `_Size` on `being_hologram.mat` (0.03 is a guess for a 14 cm sphere) | CS-175 | todo |
| CS-177 | TERM | Being, no network: press the dock button, the sphere fades in at the left and follows the head; tap it, it brightens (Listening) and goes idle after the 4 s timeout with nothing said; press the button again, it fades out; the desktop HUD button and `C` do the same | CS-176 | todo |
