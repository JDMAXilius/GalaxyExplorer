...
| CS-188 | CC | App Check's two Galaxies assertions contradict the shipped design and always fail. `DockController.Choose` deliberately opens the Galaxies *place* rather than a list — its own comment says so, and commit `8343a1b3` is titled "The Galaxies tile opens the sphere again, not a list" — but the harness branches on `HasLayoutChoice \|\| HasPlaceChoice` and then demands a `DockPopup`. Two dead things fall out of the same reading: **`ExperienceModule.HasPlaceChoice` is referenced only by the harness**, by nothing in the runtime, and **`DockPopup.Open`'s places branch is unreachable** — it early-returns unless `HasLayoutChoice`, so `showingLayouts` is always true and `_places` is always empty. Fix the harness to assert what ships, and either delete the dead branch or implement the list (owner's call — the code says the list was deliberately dropped). **Also correct CS-169's row**, which claims "done (verified by App Check)": App Check had never been run when that was written | CS-168 | done: the harness asserts the Galaxies tile opens like any tile; `HasPlaceChoice`, `DockPopup.PlaceChosen`, `_places` and the unreachable branch are deleted; `Places` stays for `GalaxyBarBuilder` |
| CS-189 | CC | App Check never places the content, so nothing that has to be *on screen* can ever be tested — which is why the destination-tag step and the galaxy-option step both SKIP rather than run. The walk ends the intro by asking for a place (step 1), which skips the floor placement, so the content root stays at the origin — and so does the camera. Measured on the 14 Sep run: `Camera.main` at `(0,0,0)`, FOV 30, aspect 1.86; all nine Milky Way markers alive with live colliders, **every one of them at viewport y between 1.28 and 1.57** (above the top of the screen) and four of them behind the camera at `z<=0`. The walk must complete the intro placement, or place the content root itself, before any step that looks for something on screen; until it does, `FindTag` returning null is the harness's own doing and not a fact about the app. Worth checking at the same time whether a 30-degree desktop FOV can frame the Milky Way map at all, the way CS-171 had to do for the Solar Row | CS-168 | done (by reading): `IntroFlow.Skip` anchors the content 2 m ahead when placement never ran, the same `AnchorAhead` the quick start uses; the 30-degree framing question is CS-190's |
| CS-190 | TERM | Run **Cosmic Simulation → Verify → App Check** again: the intro click now anchors the content (CS-189), so the Milky Way marker step must run rather than skip; the new being walk (CS-187) must pass - summon, offset, facing under a 40° turn, drag without listening, brush and spring back, tap, timeout, fade out; then answer the framing question: with the anchor 2 m ahead at eye height, does a 30° desktop camera see the markers, or does the Milky Way need the same push-back CS-171 gave the Solar Row | CS-187, CS-189 | todo |
| CS-191 | TERM | Re-bake every galaxy's `StarsData` (the galaxy builders) so the shuffled order of CS-125 lands in the assets, confirm a byte-identical second bake, and check the Milky Way, Andromeda and the three outside galaxies look unchanged at full count | CS-125 | todo |
| CS-192 | TERM | The being's look after CS-184 to CS-186 on the desktop and on the Quest: a tap blooms the touch colour where the finger landed and the bloom drifts off; while it speaks the belts move with the syllables (`articulate`, 0.35) rather than the whole ball breathing; tune `touchDrift`, `articulate` and `speakSwell` on the prefab | CS-178 | todo |
| CS-193 | TERM | The being's place and feel, owner's direction 14 Sep: it was too close and clipped at the corner. Settings now put it 1.4 m out, 0.32 m left, 0.1 m down - about 13 degrees left of centre, the whole 14 cm sphere in a 30-degree desktop view - and the same asset drives the Quest. Confirm on both: it sits mid-left and whole; a grab (pinch, or mouse press and drag) carries it and it re-hangs from wherever it was let go; a hover across it turns it and it springs back; a tap still listens | CS-187 | todo |
| CS-194 | TERM | The being, owner's direction 16 Sep: follows the head's pitch as well as its yaw (`BeingAnchor` keeps station in a yaw-and-pitch frame, no roll, and pitches to face the head); the press pop is a fixed 1 s click that always returns to the default size (`_Active` now follows the press only - held on through Listening and Speaking it kept the being swollen); a tap plays the `Select` click and speaks a stored greeting (`BeingSettings.GreetingClip`, Zira placeholder, `BeingSpeaker.Say`) and then listens, relay or not, logging what it heard; `BeingMic` gets a remembered device (`BeingMic.Device`), closes the mic on dismiss, and waits for the Android permission answer | — | done (16 Sep, editor, per-frame trace: tap to Speaking 4.3 s with bands moving, pop 0.159 back to 0.140 m inside 1 s, Listening 4 s then Idle, a 35° look up followed back to the same viewport spot in about 0.6 s. The only input device on this PC is `MOTIV Mix Virtual Output (Shure Virtual Audio)`, and nothing was said into it. Code written in the terminal session at the owner's request. Not yet seen on the Quest) |
| CS-195 | CC | P6 smoke regression (16 Sep run, `[P6] DONE 24/28`, 27/27 on 13 Sep): the desktop click on the Milky Way's `solar_system` destination tag hits `body_saturn` 7.41 m away (`3D=body_saturn, UI=nothing`, tag at screen 922, 370), so the overlay never opens and the Halo and R-restore steps fail after it. Either the solar-system content is live behind the Milky Way map when it should not be, or the harness clicks before the switch has cleared it | — | todo |
| CS-196 | CC | Settings window, owner's direction 16 Sep: add a **Microphone** row (‹ device ›, cycling `Microphone.devices` into `BeingMic.Device`, with a live level bar) and a two-tap **Quit** to `UtilityWindow`; set Mute and Narration side by side; the window grows to 120 × 130 mm; the desktop dock's Mute button becomes **Settings** (`M` still mutes). Mockup: https://claude.ai/artifact/QwVdM4XzMXSSSr7qXJjugr. Update `docs/ui/spec.md` §5 and GDD §8.2 | CS-194 | done (16 Sep, built in `main_scene`'s tree as the owner directed: `UtilityWindow` Quit, `MicrophoneRow`, `UiPrefabBuilder` 120 × 130 layout, `DesktopDock` Mute → Settings, a generated gear icon. Seen live in Play in main_scene: the desktop Settings button opens it, the row shows System default and cycles to the one device here, the meter runs, text size and narration apply, Quit arms, disarms after 3 s and stops Play on the second tap. App Check 31/32 before a harness timing fix to its tap step; the fix is not re-run yet. Level bar not seen moving: nobody spoke into the one device, a virtual output. Not seen on the Quest. The same panel and the being were also ported to the Cosmic tree (`Assets/Cosmic/Being`, `UtilityUi`, `MicPicker`) for the cutover; that port passed its own Play check but its selected-button labels stay white) |
| CS-197 | TERM | The being, owner's direction 16 Sep (photo beside the intro ball): 0.20 m wide, built from the intro ball's own `placement_object_scatter_mesh` (6369 points); at rest `_Density` 0.08 shows a sparse share, and while speaking every point shows and falls **inward** (`_Inward`, 0.35 + 0.45 × loudness, per point depth, harder on a loud belt) so it fills in like the intro ball - the outward swell and belt push are off. The placement shader gains `_Density` (default 1) and `_Inward` (default 0), so the intro is unchanged. Also fixes a size bug: the built-in sphere is two units across, so the being was 0.28 m with a 0.14 m collider; every child is now scaled from its mesh's bounds and the collider matches. Greeting is now Higgsfield `seed_audio` voice Holden, trimmed to 3.5 s; the intro narrator was not cloned (a real person's voice, see CREDITS) | CS-194 | done (16 Sep, editor, main_scene: built 0.200 m, collider r 0.1; a tap went Speaking with density 1.0 and inward 0.35-0.80 following the voice; frames captured at rest and speaking. Not compared on screen with the intro ball in the same frame - the intro was already over. Not seen on the Quest) |
| CS-198 | CC | Carry CS-197 into the Cosmic port: `Cosmic/BeingPoints` needs `_Density` and `_Inward`, `BeingLook` the inward pose instead of the swell, and `Editor/BeingPrefab` the scatter mesh at 0.20 m scaled from bounds - it has the same built-in-sphere size bug (0.28 m drawn, 0.14 m collider). Bake the scatter mesh into `being_points.asset` so the cutover can delete the intro model | CS-197 | todo |
| CS-199 | TERM | Galaxies like the Cosmic Web, owner's direction 16 Sep: start in the middle of the shell, with the info card inside it. `GalaxyField` centred its shell on the head at spawn, then `ExperienceDirector.FrameContent` re-centred the content on the root 2 m ahead (shell ended 2 m off the head) and measured it, which pushed the scene panel to x = 4.75 m, off screen. Content whose `GalaxyField.CentresOnViewer` is set is now neither framed nor measured, the desktop orbit pivots on the shell centre, and the named-galaxy pins node follows the shell centre so each pin still hangs 2.85 m out, just inside the shell. Known and accepted: during the grow-in the shell's centre slides from the root onto the head, because its offset scales with the root | — | done (16 Sep, Play in main_scene: shell centre 0.00 m from the camera, all four pins 2.85 m from it, panel at (0.47, 0.20, 2.00), the same spot as the Cosmic Web's, on screen; App Check `[APP] DONE 32/32`, its first full pass, which also confirms CS-196's tap-timing fix. Not seen on the Quest) |
...
| CS-173 | CC | Touch nudge on every body: brushing a planet, the Sun or a moon with a fingertip, a hand ray or the mouse (without grabbing) turns it a little with the stroke and springs it back; `TouchNudge`, added to every `ForceSolver` at run time | — | done (by reading; needs the editor pass) |
| CS-174 | CC | Tap a pulled body to send it home: a click or pinch on a planet already out (pressed and released without dragging) returns it to the row with its moons; the same on a pulled moon returns it to orbit round its planet wherever the planet is; `ForceSolver.ReturnHome`, shared with the number keys | — | done (by reading; needs the editor pass) |
| CS-175 | CC | Cosmic Being, phase 1+2 code: `Assets/scripts/being/` (CosmicBeing, BeingAnchor, BeingVisual, BeingMic, BeingSpeaker, BeingLink, BeingContext, BeingActions, BeingSettings, Editor/BeingBuilder), the dock and desktop-dock button, `C` on desktop, the relay `tools/being-relay/` (type-checks clean, knowledge parser run against `Assets/data`) | — | done (by reading; the relay compiles) |
...
**Cloud session, 14 Sep 2026 — the hand-off queue worked.** CS-184, CS-185, CS-186, CS-187, CS-188, CS-189, CS-118, CS-124 and CS-125 are done by reading; CS-167 is blocked on one line of editor output (its row says which). Three things worth knowing beyond the rows. The press feedback and the articulation both live in the intro's placement shader, which now has `_Bands`, `_Articulate` and `_BandExtent`, all defaulting to zero so the intro object is untouched; the being sets them through its instanced material. `CosmicBeing` now forwards press and release to its `ManipulationHandler`, the way a body's `ForceSolver` does - without that the handler never heard a press and a drag could not have carried it, whatever CS-182 hoped. And App Check's being walk drives everything through queued mouse events with the flush, so it proves the pointer path and not the handlers.
...
| CS-124 | CC | `DrawStars` scales badly: `AttachAllInOrder` re-attaches every command buffer on every camera whenever a drawer appears (quadratic in drawer count, and it fires in the frame the player pokes a tile), and `Update` writes ~10 material properties per layer per frame | — | done (by reading): a new drawer re-attaches only the drawers after it, and the two shape vectors are written on change; the camera-relative values still write every frame because they change every frame. **It did not compile as written** (fixed 15 Sep 2026): `_fuzzy` was declared `Vector4`, which made `galaxy.FuzzySideScale != _fuzzy` ambiguous against the `Vector2` — both convert to the other. The field is a `Vector2` now and `SetVector` widens it, which is what the `Vector4` held anyway |
| CS-125 | CC | Galaxy LOD by vertex-count truncation is free but currently impossible: `SpiralGalaxy.GenerateEllipses` emits inner-to-outer and concatenates the second arm after the first, so drawing fewer points deletes the outer arms and one whole arm. Shuffle the bake order so a truncated draw thins the galaxy evenly instead | — | done (by reading): `CreateStarsContent` shuffles with a seed from the counts, so a re-bake is byte-stable; the shipped `StarsData` assets keep the old order until CS-191 re-bakes them |
...
**Phase 3 note (14 Sep 2026, CS-171: every planet in the row is in the desktop view).**

*The owner's report:* in the size layout the row was cut off at Pluto. *The cause is the desktop camera, not the layout.* `main_camera_prefab` has a 30-degree vertical field of view (about 51 degrees across at 16:9), the content root sits 2 m in front of it, and the camera never moves or turns on a monitor: `DesktopMouseInput`'s orbit, pan and zoom drive `TouchController`/`Pivot` under the `Loader`, and nothing hangs off that pivot any more (its only child is `BoundingBox`), so the wheel and the drags do not reach the content either. Solar Row is 1.9 m wide with its ends bowing 0.4 m nearer, exactly the width the view has at 2 m; Relative Size is five metres wide with a 3 m Sun. Both overflowed on both sides.

*The fix, `ExperienceDirector.FitArrangement`:* desktop only (`XRSettings.isDeviceActive` false — in a headset the player turns, which is what GDD 4.1's arc is for). For content carrying a `LayoutRig`, the box of the **target** arrangement is read off the preset's slots (`LayoutRig.TryGetLayoutBounds`, each body at its full span at the diameter the layout gives it — so it is right at the start of a move, not only at its end), centred on the content root the way `FrameContent` centres the first arrangement, and then pushed straight away from the camera until all eight corners sit inside `desktopFitMargin` (0.85) of the view's half-width and half-height. Never pulled nearer. At spawn it snaps before `CaptureHome` and the grow-in, so the grow-in keeps it; on a later pick from the dock's pop-up (or R) the director subscribes to the rig's `LayoutApplied` and moves the content over the preset's own 0.8 s with the same ease-out, then re-captures the content's home so Restore returns to where the row fits. For Relative Size that is roughly six metres back: a 3 m Sun in a 30-degree view cannot be nearer, and Pluto is a 5 mm dot at that distance by the layout's own definition. Widening the desktop camera's field of view would halve that and is the owner's call; nothing here depends on it.

*What the editor pass must confirm:* open Solar System Planets on the desktop — all ten bodies visible with a border, the row centred; pick Relative Size from the dock pop-up — the content eases back over 0.8 s and the whole row, Sun to Pluto, stays in view; pick Solar Row again — it eases forward; press R in either — nothing jumps; a headset session is unchanged.

**Note (14 Sep 2026, CS-172 and CS-173: the desktop camera after travelling, and the touch nudge).**

*The owner's report:* open a nebula scene in the editor and play, the inputs work; travel to the same nebula from the Milky Way and "the inputs for moving the camera, all the inputs" do nothing. *What was actually happening:* nothing is lost in the travel, and nothing needed re-initialising. The standalone nebula scenes (`NebulaSceneBuilder`) put `FreeLook` on their camera — a fly camera the app never had. In the app, `DesktopMouseInput`'s orbit, pan and zoom turned and slid `Loader/TouchController/Pivot`, the pivot the original app's camera hung off; the camera is the XR rig's now and the content sits under the Loader beside that pivot, whose only child is `BoundingBox`. So every drag and every wheel notch on empty space moved a transform nothing hangs off, in every place — on the Milky Way it went unnoticed because the galaxy answers a drag on itself, and inside a nebula, with nothing under the pointer, it was the first thing tried. Clicks, hovers and the dock were never affected; `TransitionManager.SetActivationOfTouchscript` only runs on its own scene loads, which a prefab-backed place never makes, so `InputEnabled` stays true across a switch.

*The fix (CS-172).* The three view controls move the **camera rig** (`Camera.main.transform.root`, the `main_camera_prefab` instance at the scene root) and nothing else, with FreeLook's conventions so the app feels like the scenes the owner judged it against: left drag yaws about the world's up and pitches about the camera's right (clamped to ±85°, no roll), about the pivot `ExperienceDirector.TryGetViewPivot` reports — the open content's `CentreOnViewer` position for a place the player stands inside, which makes the drag a look-around, or the content root for the galaxy and the solar system, which makes it an orbit; right drag slides the camera so the world follows the mouse; the wheel flies forward and back along the view (0.25 m a notch, was 0.1 on a dead pivot); Home puts the rig back where the intro left it. Headset sessions are untouched: `Rig()` answers null while `XRSettings.isDeviceActive`. The legacy pivot reset is kept in `ResetView` for the `TransitionManager` code that still reads it.

*The touch nudge (CS-173).* `TouchNudge` (`Assets/scripts/experience/TouchNudge.cs`) is added to every `ForceSolver` in its `Awake`, so the ten orbit-model planets, the Sun, every moon and the ten row bodies all get it with no prefab edit; one placed on a prefab by hand keeps its own tuning. It is an `IGEFocusHandler` on the solver's object, which is where `GEInputEvents.ExecuteHierarchy` already delivers a body's focus, so a fingertip (near hover), a hand ray and the mouse all count. While focused and the body is at rest (Root, Free — a body being pulled or held is ignored), the pointer's travel across the body along the camera's right axis becomes a turn: 70° per radius of stroke, capped at 40°, right-to-left turning the face right-to-left; a spring (stiffness 70, damping 11, a little under critical so it overshoots once) pulls it back to zero whether or not the finger is still there. It never writes a rotation: it turns the body's `*_tilt` node (or its first child) by the change in angle each frame about the world's up, so `ConstantRotateAxis`'s spin composes with it and when the spring lands the net rotation is exactly none; disabling it undoes whatever is still applied. No haptics exist for tracked hands; the bounce is the feedback.

*What the editor pass must confirm:* on the desktop, travel Milky Way → any nebula, then left-drag (look around), wheel (fly in and out), right-drag (slide), Home (back); on the Milky Way a left-drag orbits the galaxy; with the mouse over a planet in the Solar System and the Planets row, moving across it turns it and it springs back, and a pulled planet does not react; on the Quest, brushing a planet with a fingertip does the same; console clean.

**Note (14 Sep 2026, CS-174: tap a pulled body to put it back).**

*The owner's ask:* click a planet in the row and it flies to the centre of the view (already so, and liked); click that centred planet again and it should go back to its place in the row, moons with it; with a planet centred, click one of its moons and the moon comes in front (already so); click that moon again and it should go back to orbiting the planet, which stays centred; and clicking the centred planet then sends it home.

*How it works.* A press on a body that is already **Free** (out of the arrangement) arms a tap in `ForceSolver.OnPointerDown` — pointer, time, position — and starts the grab exactly as before. On `OnPointerUp`, after the grab has ended and the body is Free again, a press that was shorter than `TapSeconds` (0.4 s) and moved the body less than `TapMoveMetres` (3 cm) is a tap, and the body goes home through `ReturnHome`: its `FreePlacementSolver.RestoreLayout` walks it to its home anchor over the layout's own time and lands it in Root; a body without one (the orbit model's planets and moons) goes straight to Root. A moon's home anchor sits under its planet's body, so a moon returns to its orbit round the planet wherever the planet is now — centred or in the row — and a planet returning to the row carries its riding moons with it, as it does today. A press that lasts longer or carries the body is a grab and leaves the body where it is let go, as GDD 4.1 asks. The desktop number keys' "press again to return" (`DesktopMouseInput.ToggleBody`) now goes through the same `ReturnHome`, so key and click cannot disagree. Mouse, hand ray and near pinch all arrive as the same pointer down/up pair, so nothing is desktop-specific.

*What the editor pass must confirm:* Planets row on the desktop — click Earth (flies to centre), click Earth (returns to the row, Moon with it); click Earth, click the Moon (comes in front), click the Moon (back into orbit round the centred Earth), click Earth (home); drag a pulled planet and release — it stays; on the Quest the same with pinches; the orbit-model Solar System's planets and moons behave the same through their own solvers.

**Note (14 Sep 2026, CS-175: the Cosmic Being, phases 1 and 2 written).**

*What it is:* see `docs/COSMIC_BEING.md`. *What was decided in the planning:* the Claude API is text only (checked against the live feature list: no speech in or out, streaming is SSE), so OpenAI is the ears (Whisper transcription of one utterance) and the voice (streamed PCM); both keys live in a Node relay under `tools/being-relay/`, never in the build; tap-to-talk on the sphere rather than an open microphone, ending on 1.2 s of silence; the visual is the intro's `PlacementObject` point cloud over a built-in sphere with the intro's blue material and rim, so no new shader and no new mesh. The relay reads the module and body `.asset` files directly for its knowledge, so there is no copied text to drift. Defaults taken for the two open answers: the relay runs on the owner's machine (`ws://127.0.0.1:8787/being`, change it on the settings asset for the Quest), and the voice is `coral` in `.env`.

*Written by reading, in the cloud session:* every C# file, the dock edits (`DockController.ToggleBeing`, a fifth small button under the world dock at x = 372, a fifth icon button on the desktop HUD dock with `ButtonCount` raised so the plate widens, `C` on the keyboard), and the relay, which was installed and type-checked here (`tsc` clean on `@anthropic-ai/sdk` 0.125 and `openai` 7.15) with the knowledge parser run against the real assets. What could not be done here is everything that needs the editor or a key, which is CS-176 to CS-181.

## Phase 4 — Milky Way destinations, nebulae, black hole

| ID | Track | Title | Depends | Status |
|---|---|---|---|---|
| CS-050 | TERM | Replace galaxy POI cards with `LabelButton` tags (6 + 3 extras on) — **and remove the legacy `CardPOI` / `poi_text_card_*` path once tags replace it**, per roadmap §4.3 and Technical Overview §15, both of which already call it retired | CS-026, CS-032 | superseded by CS-170 (owner, 14 Sep): the original markers are the product's map again |
...
**Phase 4 note (14 Sep 2026, CS-170: the original markers are back, hover for the card, click to travel).**

*What the owner asked for, in their words:* the Milky Way scene reverted to the original Galaxy Explorer look — the angled leader lines with "SOLAR SYSTEM / OUR NEIGHBORHOOD", "GALACTIC CENTER / BLACK HOLE", "CRAB NEBULA", "PILLARS OF CREATION", "HOMUNCULUS NEBULA", "NGC 1501", "TRUMPLER 14" — with our code on it: every nebula present (seven), the picture card on **hover** instead of on click, and a **click travels** to the nebula's scene.

*What changed.* The markers were never gone: `galaxy_pois_prefab` still held the five `poi_prefab` instances and the `solar_system` large marker, `galaxy_view_scene` the loose `galactic_center` one; CS-050 had switched them off in `CardPOI.Awake` and `LegacyPoiCleanup` had deactivated the two large ones. The switch is gone from `CardPOI`, the two large markers are active again (the prefab override and the scene override), and **two new `poi_prefab` instances, `poi_helix` and `poi_orion`,** are written into the prefab at the positions `DestinationTagBuilder` gave those two tags, with the nebulae plates (`Assets/Textures/nebulae/helix_texture.jpg`, `orion_texture.jpg`) as their card photographs, their module narration as `CardAudio`, and distance / size / age texts. Their pointing-line positions are not baked into the prefab because `DynamicLine` rewrites them every frame. The `destination_tags` node and its nine `label_button_prefab` tags are **removed from the prefab** (48 YAML documents, no dangling reference left), and `DestinationTagBuilder`, `DestinationTags` and `LegacyPoiCleanup` are emptied to tombstones — the owner does not want the terminal's tags on the map, and a menu item that could rebuild them would only invite it. The three empty files are still on disk because the cloud session was not allowed to delete files; **delete them with their `.meta` from the terminal.** `LabelButton` and `label_button_prefab` stay: `GalaxyPins` still uses them.

*How the card and the click work now.* `CardPOI` implements the marker two ways at once: `OnFocusEnter` (hand ray or mouse over the label) opens the card at the marker's base facing the player; `OnFocusExit` closes it after a 0.35 s grace so skimming the label's edge does not flicker; `OnPointerDown` hides the card and calls `ExperienceDirector.Switch` on the marker's `destination` module (a serialized `ExperienceModule`, with `destinationId` as a fallback looked up on the director). The card never sets `IsCardActive`, so `CardPOIManager` no longer freezes the galaxy's spin or disables the other markers' colliders, and the card's own colliders (close button, plate) are switched off once in `Start` so the card cannot steal the hover that opened it. `PlanetPOI` (Solar System, Galactic Center) got the same `destination` field, falling back to the module whose `SceneName` equals `SceneToLoad`, and goes through `Switch` when a director exists; the original `TransitionManager` path is kept for a director-less scene. Switching to a nebula module is the terminal's "nebula is a place" design (CS-169's shape): the module's `ContentPrefab` is the nebula scene; the `nebula_scenes/*.unity` files are not in Build Settings and are not what a click opens.

*Why both mouse and hand work with no special case.* The label's `BoxCollider` sits on `TextArea`, whose `GEInteractable` raises focus and pointer events through `GEInputEvents.ExecuteHierarchy`, which walks up to the `POI` object carrying `CardPOI`. `DesktopMouseInput.SetHovered` raises the same `RaiseFocusEnter` / `RaiseFocusExit` a hand ray does, and a mouse press raises `RaisePointerDown`, so the desktop path is the XR path.

*What the editor pass has to confirm (CS-168's App Check; `FindTag` now finds a marker's collider):* nine markers drawn on the Milky Way, hovering any nebula label shows its card and leaving hides it, clicking each of the seven nebulae opens that place, clicking Solar System and Galactic Center opens those scenes, the galaxy keeps spinning while a card is up, console clean. Not verifiable here: this is a cloud session with no editor, so the YAML was written by hand and the C# checked by reading.
...
| CS-118 | CC | `DockController.SetVisible` never toggles the `plate` Image, so a hidden dock still shows its dark plate floating in the room | CS-028 | done (by reading): `SetVisible` finds the builder's `plate` child by name, so no prefab rebuild |
