...
| CS-060 | CC | Solar Row: Saturn and Uranus ring spans exceed the 25 cm pitch and intersect by 6.9 cm - decide pitch vs ring span | CS-041 | todo |
...
| CS-062 | CC | Player-visible message when an experience fails to load (today it is silent, log only) | CS-037 | todo || CS-039 | TERM | Wire the Earth atmosphere shell (1.025 sphere child + `SunLightReceiver`) onto the Earth prefab — nothing references `earth_atmosphere_material` yet | CS-047 | todo |
...
*Open decision, CS-060:* Saturn's rings span 33.9 cm and Uranus's 29.9 cm, against a 25 cm pitch — **they intersect by 6.9 cm**, and it is visible. The GDD fixes the pitch and the planet diameter but says nothing about ring span, so this is the spec's consequence rather than a builder error. It needs a call: widen the pitch, or shrink the ring meshes in this arrangement only.
