...
| CS-124 | CC | `DrawStars` scales badly: `AttachAllInOrder` re-attaches every command buffer on every camera whenever a drawer appears (quadratic in drawer count, and it fires in the frame the player pokes a tile), and `Update` writes ~10 material properties per layer per frame | — | done (by reading): a new drawer re-attaches only the drawers after it, and the two shape vectors are written on change; the camera-relative values still write every frame because they change every frame |
