...
| CS-144 | CC | Rework P4d: `Editor/Content.cs` + `Bodies.cs` menus **Cosmic → Build → Bodies / Places** (every Place asset → one prefab, every Body → one nested prefab, materials from the surveyed values); `Content/Field.cs` + `Field.shader` (deep-field sprite shell); `BlackHole.shader` port; P4 harness (`Cosmic/Verify/P4 …`, `tools/mcp/rework/p4_*.cs`) | CS-142, CS-143 | doing |
...
| CS-150 | CC | Rework P4 debt: `WebSource.cs` 294 lines, `Orbit.cs` 308, `Layouts.cs` 324 against the ~250 budget; editor scripts at 9 against the table's 6 (Bake split three ways, Bodies); `Content/Field.cs` reads plates through a blit at enable; the Milky Way dust layer lost its own winding (540°) and radii (0.9/1.1) because `Galaxy` holds one pair per galaxy; the pull beam material still points at the old shader | CS-144 | todo |
