...
- Relay: `%USERPROFILE%\.unity\relay\relay_win.exe --mcp --project-path <repo>`; tools `Unity_RunCommand`, `Unity_GetConsoleLogs`, `Unity_Camera_Capture`. The tooling is now checked in at **`tools/mcp/`** (outside `Assets/`, so Unity never compiles it into the build) — `umcp.js` (`list`, `call <tool> <json>`, `run <file.cs>`), `compile.ps1` (refresh + wait + errors from `Logs/Editor.log`), `smoke.cs`, `enter_play_mode.cs` / `leave_play_mode.cs`, and a README. Stop recreating it in a scratchpad; read `tools/mcp/README.md` first. None of it has been run against the real relay yet.
...
## Build
Menu **Cosmic Simulation → Quest 3**: *Configure Project*, *Build APK* (→ `Builds/Quest3/CosmicSimulationXR.apk`), *List OpenXR Features*, *Apply Android Asset Overrides*. `Quest3ProjectSetup.ConfigureProject()` runs first on every build and **writes player settings from its own constants** (bundle id, SDK levels, graphics API) — change identity there, not only in Project Settings, or the build silently reverts it. An unattended build stops on the modal *"Unsupported Input Handling on Android"* (we need Active Input Handling "Both"): answer **Ignore**. **Stereo mode: this line was wrong and is under correction — see `docs/SHADER_STEREO_AUDIT.md`.** It used to read "Android multiview; Windows/Link multi-pass". `Assets/XR/Settings/OpenXR Package Settings.asset` stores `m_renderMode: 1` for Android and `0` for Standalone, i.e. the two platforms genuinely differ, and if Unity's enum is `SinglePassInstanced = 0, MultiPass = 1` then the old line had them exactly backwards: Android would be multi-pass and Link single-pass instanced. The OpenXR package source is not vendored, so **confirm the enum in the editor before relying on either reading**. This matters because most shaders here had no stereo macros at all, which is invisible under multi-pass and breaks one eye under single-pass instanced. All custom shaders need stereo macros, no geometry shaders, target ≤ 4.5.
